...
class ArrayUtil {
  static filterArrayByString(mainArr, searchText, fieldsToSearch) {
    if (searchText === '') {
      return mainArr;
    }
    ...
    return mainArr.filter((itemObj) =>
      this.searchInObj(itemObj, searchText, fieldsToSearch)
    );
  }

  static searchInObj(itemObj, searchText, fieldsToSearch) {
    for (const prop in itemObj) {
      if (
        (fieldsToSearch && !fieldsToSearch.some((x) => x === prop)) ||
        !itemObj.hasOwnProperty(prop)
      ) {
        continue;
      }
